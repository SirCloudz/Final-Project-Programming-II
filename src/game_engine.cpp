...
void GameEngine::end_turn()
{
    ...
    {
        // insufficient food: reduce morale (not implemented) and disable random units (simple: clear last unit)
        int shortage = consume - comida;
        comida = 0;
        // remove up to shortage units (simple)
...
{
    // basic guardian movement: move any J2 unit one cell to the right if possible (wrap disabled)
    for (int r = 0; r < map.num_rows(); ++r)
    {
        for (int c = map.num_cols() - 1; c >= 0; --c)
        { // iterate right-to-left to avoid double moves
            Tile &t = map.at(r, c);
